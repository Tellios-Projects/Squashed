...
public class ModBlocks {
    ...
    public static final Block SUGAR_BLOCK = registerBlock("sugar_block", new FallingBlock(FabricBlockSettings.create().mapColor(MapColor.OFF_WHITE).strength(0.5f).sounds(BlockSoundGroup.SAND).burnable())/*,ModItemGroups.SQUASHED*/);
    ...
    public static final Block EXPERIENCE_BLOCK = registerBlock("experience_block", new Block(FabricBlockSettings.create().mapColor(MapColor.YELLOW).strength(0.2f).sounds(BlockSoundGroup.AMETHYST_CLUSTER).emissiveLighting(ModBlocks::always).luminance(state -> 1))); //TODO consider making a custom BlockSoundGroup for this
...
//    }
    ...
    public static Block createCompressedLeavesBlock(MapColor mapColor, BlockSoundGroup blockSoundGroup) {
        return new Block(FabricBlockSettings.create().mapColor(mapColor).strength(0.2f).sounds(blockSoundGroup).nonOpaque().allowsSpawning(ModBlocks::never).suffocates(ModBlocks::never).blockVision(ModBlocks::never).burnable().pistonBehavior(PistonBehavior.DESTROY).solidBlock(ModBlocks::never));
    }
